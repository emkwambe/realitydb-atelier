...
// Shared CEO-briefing editor. Receives only slimmed, client-safe data as props
// (no rubric hiddenStory/failCriteria, no exercise referenceSQL). The server
// briefing page supplies these after gating, so sensitive content stays on the
// server.
...
export function BriefingForm({
  ...
  const STORAGE_KEY = `atelier:${company}:briefing`;
  const RESULT_KEY = `atelier:${company}:result`;
  const DATASET_KEY = `atelier:${company}:dataset`;
  const VISITED_KEY = `atelier:${company}:visitedScenarios`;
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(`/companies/${company}/results`);
    } catch (e) {
      ...
      <main className="flex-1 bg-[#0a0f1a]">
        <div className="mx-auto max-w-3xl px-6 py-10">
          <header>
            ...
            <h1 className="mt-2 text-3xl font-medium text-[#e2e8f0]">{title}</h1>
            <p className="mt-2 text-sm text-[#64748b]">{intro}</p>
          </header>

...
interface Props {
  exercises: Exercise[];
  company: string;
  // Difficulties the viewer can't access (computed server-side from canAccess).
  // Empty while ENABLE_PAYWALL=false. Locked rows show a badge but still link —
  // the exercise route renders the upgrade gate.
  lockedDifficulties?: string[];
}

export function ExerciseNavStatic({
  exercises,
  company,
  lockedDifficulties = [],
}: Props) {
  return (
    <ol className="divide-y divide-[#1e293b]">
      {exercises.map((ex) => {
        const locked = lockedDifficulties.includes(ex.difficulty);
        return (
          <li key={ex.id}>
            <Link
      ...
      <li>
        ...
        >
          ...
          {lockedDifficulties.includes("advanced") ? (
            <Lock className="size-4 text-[#06d6a0]" />
          ) : (

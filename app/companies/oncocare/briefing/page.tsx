...
const STORAGE_KEY = "atelier:oncocare:briefing";
const RESULT_KEY = "atelier:oncocare:result";
const DATASET_KEY = "atelier:oncocare:dataset";
const VISITED_KEY = "atelier:oncocare:visitedScenarios";
...
export default function OncoCareBriefingPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      router.push("/companies/oncocare/results");
    } catch (e) {
            ...
            <div>
              ...
              {rubricOpen && (
                <ul className="mt-2 space-y-2 text-[11px] leading-relaxed text-[#64748b]">
                  {Object.values(oncocareRubric.axes).map((axis) => (
                    <li key={axis.name}>
                  ...
                  <li className="pt-2 text-[#06d6a0]">
                    Passing score: {oncocareRubric.passingScore}/100
                  </li>
                </ul>
              )}
            </div>
        ...
        <div className="mx-auto max-w-3xl px-6 py-10">
          <header>
            <div className="font-mono text-xs uppercase tracking-wider text-[#64748b]">
              OncoCare Therapeutics · Module 06
            </div>
            <h1 className="mt-2 text-3xl font-medium text-[#e2e8f0]">
              CMO Recommendation Memo — ONC-441 Phase III
            </h1>
            <p className="mt-2 text-sm text-[#64748b]">
              FDA advisory meeting in 6 weeks. 700–900 words. Four-part
              structure below. Cite specific exercises, name the FDA 50%
              threshold and the ICH E6 GCP standard, and quantify the
              site-level impact on the trial-level result.
            </p>
          </header>
          ...
          <div className="mt-6">
            <textarea
              ...
              spellCheck
              placeholder={`1. Diagnosis (which site is the outlier, with evidence)…\n\n2. Intervention (scenario tested)…\n\n3. Decision (exclude SITE-07 vs remediate)…\n\n4. Epistemic honesty (alternative explanations, additional data needed)…`}

...
const RESULT_KEY = "atelier:oncocare:result";
const CERT_KEY = "atelier:oncocare:cert";

interface AxisScore { score: number; feedback: string }
  ...
  useEffect(() => {
    ...
    (async () => {
      const c: BizCertificate = {
        ...
        company: oncocareRubric.company,
        companyLabel: oncocareRubric.companyLabel,
      ...
      setCert(signed);
      try { localStorage.setItem(CERT_KEY, JSON.stringify(signed)); } catch {}
    })();
    ...
    return (
      <div className="mx-auto max-w-2xl px-6 py-20 text-center">
        ...
        <p className="mt-2 text-sm text-[#64748b]">
          Submit a CMO recommendation memo to see your score and certificate.
        </p>
        <Link
          href="/companies/oncocare/briefing"
          className="mt-6 inline-flex items-center gap-2 bg-[#06d6a0] px-4 py-2 text-sm font-medium text-[#0a0f1a]"
        >
          Go to memo <ArrowRight className="size-4" />
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
        ...
        <div className="flex items-center gap-4">
          <div
            className={`flex size-12 items-center justify-center border ${
              result.passed ? "border-[#06d6a0] text-[#06d6a0]" : "border-[#ef4444] text-[#ef4444]"
            }`}
          >
            {result.passed ? <CheckCircle2 className="size-6" /> : <span className="font-mono text-xl">×</span>}
          </div>
          <div>
            <div className="font-mono text-xs uppercase tracking-wider text-[#64748b]">
              CMO Memo · OncoCare Therapeutics
            </div>
            <h1 className={`text-2xl font-medium ${result.passed ? "text-[#06d6a0]" : "text-[#ef4444]"}`}>
              {result.passed ? "Passed" : "Did not pass"}
            </h1>
          </div>
          <div className="ml-auto text-right">
            <div className="font-mono text-[11px] uppercase tracking-wider text-[#64748b]">Score</div>
            <div className="font-mono text-4xl font-medium text-[#e2e8f0]">
              {result.overall_score}<span className="text-base text-[#64748b]">/100</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mt-8">
        <h2 className="text-sm font-medium uppercase tracking-wider text-[#64748b]">Per-axis breakdown</h2>
          ...
          <table className="data-grid w-full font-mono text-[13px]">
            ...
            <tbody>
              {(["segmentation","causal_reasoning","quantification","recommendation","epistemic_honesty"] as const).map((key) => {
                ...
                const def = oncocareRubric.axes[key];
                ...
                return (
                  <tr key={key} className="border-t border-[#1e293b]/60">
                    ...
                    <td className={`px-4 py-2 text-right ${
                      axis.score >= def.maxScore * 0.7 ? "text-[#06d6a0]"
                        : axis.score >= def.maxScore * 0.5 ? "text-[#f59e0b]"
                        : "text-[#ef4444]"
                    }`}>
                      {axis.score}/{def.maxScore}
                    </td>
                    <td className="px-4 py-2 text-[#e2e8f0]/80">{axis.feedback}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <section className="mt-8">
        <h2 className="text-sm font-medium uppercase tracking-wider text-[#64748b]">Memo feedback</h2>
        ...
        <section className="mt-8 border border-[#06d6a0]/40 bg-[#06d6a0]/5 p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <div className="mt-1 font-mono text-xl text-[#e2e8f0]">{cert.certId}</div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a
                href={linkedInShareUrl(cert, typeof window !== "undefined" ? window.location.origin : "")}
                target="_blank"
      ...
      {!result.passed && (
        <section className="mt-8 border border-[#ef4444]/30 bg-[#ef4444]/5 p-6">
          <p className="text-sm text-[#e2e8f0]">
            ...
            Your score: <span className="font-mono text-[#ef4444]">{result.overall_score}</span>.
          </p>
          <p className="mt-2 text-sm text-[#64748b]">Retake available in 24 hours.</p>
        </section>
      )}
    </div>
  );

...
const STORAGE_KEY = "atelier:clearbank:briefing";
const RESULT_KEY = "atelier:clearbank:result";
const DATASET_KEY = "atelier:clearbank:dataset";
const VISITED_KEY = "atelier:clearbank:visitedScenarios";
...
export default function ClearBankBriefingPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      router.push("/companies/clearbank/results");
    } catch (e) {
            ...
            <div>
              ...
              {rubricOpen && (
                <ul className="mt-2 space-y-2 text-[11px] leading-relaxed text-[#64748b]">
                  {Object.values(clearbankRubric.axes).map((axis) => (
                    <li key={axis.name}>
                  ...
                  <li className="pt-2 text-[#06d6a0]">
                    Passing score: {clearbankRubric.passingScore}/100
                  </li>
                </ul>
              )}
            </div>
        ...
        <div className="mx-auto max-w-3xl px-6 py-10">
          <header>
            <div className="font-mono text-xs uppercase tracking-wider text-[#64748b]">
              ClearBank Financial · Module 05
            </div>
            <h1 className="mt-2 text-3xl font-medium text-[#e2e8f0]">
              SAR Recommendation Memo — ClearBank CCO
            </h1>
            <p className="mt-2 text-sm text-[#64748b]">
              FinCEN exam in 60 days. 700–900 words. Four-part structure
              below. Cite specific exercises, name the regulatory basis
              (BSA 31 USC 5324, 31 CFR 1020.320), and quantify the
              regulatory exposure.
            </p>
          </header>
          ...
          <div className="mt-6">
            <textarea
              ...
              spellCheck
              placeholder={`1. Diagnosis (pattern in baseline data)…\n\n2. Intervention (scenario tested)…\n\n3. Decision (file SAR vs enhanced monitoring)…\n\n4. Epistemic honesty (alternative explanations, documentation needed)…`}

...
export default async function BriefingPage() {
  ...
  return (
    <BriefingForm
      company={COMPANY}
      moduleLabel="SupplyLink Operations · Module 03"
      title="CEO Briefing — SupplyLink Supply Chain"
      ...
      parts={PARTS}
      exercises={supplylinkExercises.map((e) => ({ id: e.id, title: e.title }))}
      scenarios={SUPPLYLINK_SCENARIOS.map((s) => ({ id: s.id, shortLabel: s.shortLabel }))}
      rubricAxes={Object.values(supplylinkRubric.axes).map((a) => ({
        name: a.name,
        maxScore: a.maxScore,
        passCriteria: a.passCriteria,
      }))}
      passingScore={supplylinkRubric.passingScore}
    />
  );
}
